...
import Utility (impossible, todo)
...
toJsonSchema_Cons_final = case toJsonSchema_Cons @a of
  ...
  where
  f con = con.value # maybe (mkStringConstSchema con.type) \value -> encodeJson { type: mkStringConstSchema con.type, value }
...
instance DecodeJsonFromSchema String where
  decodeJsonFromSchema x = decodeJson x
...
instance DecodeJson (Record r) => DecodeJsonFromSchema (Record r) where
  decodeJsonFromSchema x = decodeJson x
...
decodeJsonFromSchema_Cons_final json = do
  let
    ...
    decodings = case decodeJson @{ type :: String, value :: Json } json of
      ...
      Left _ -> case decodeJson @{ type :: String, const :: String } json of
        Right result -> decodeJsonFromSchema_Cons $ StringConst result.const
...
instance IsSymbol name => DecodeJsonFromSchema_Cons (Constructor name NoArguments) where
  decodeJsonFromSchema_Cons (StringConst s) | reflectSymbolAsTag @name == s = [ Right $ Constructor NoArguments ]
...
mkStringConstSchema ∷ String → Json
