...
instance
  ( IsSymbol tag
  , RowToList r l
  , EncodeJson_TaggedUnion tag r l
  ) =>
  EncodeJson (TaggedUnion tag r) where
...
instance
  ...
  ( IsSymbol tag
  , IsSymbol k
  , Cons k (PartialRecord v) r_ r
  , Lacks tag v
  , EncodeJson (PartialRecord v)
  , EncodeJson_TaggedUnion tag r_ l
  ) =>
  EncodeJson_TaggedUnion tag r (RL.Cons k (PartialRecord v) l) where
  encodeJson_TaggedUnion = on (Proxy @k)
    ( encodeJson
        >>> Json.toObject
        >>> maybe' (\_ -> unsafeCrashWith "impossible")
...
instance
  ( IsSymbol tag
  , RowToList r l
  , DecodeJson_TaggedUnion tag r l
  ) =>
  DecodeJson (TaggedUnion tag r) where
  decodeJson json = do
...
instance
  ...
  , IsSymbol k
  , Cons k (PartialRecord v) r_ r
  , DecodeJson (PartialRecord v)
  , DecodeJson_TaggedUnion tag r_ l
  ) =>
  DecodeJson_TaggedUnion tag r (RL.Cons k (PartialRecord v) l) where
  decodeJson_TaggedUnion tag o =
    if tag == reflectSymbol (Proxy @k) then
      inj (Proxy @k) <$> decodeJson (o # Json.fromObject)
    else
